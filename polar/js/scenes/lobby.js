...
const SEAT_LABEL = { human: 'lobby.seat.human', ai: 'lobby.seat.ai', empty: 'lobby.seat.empty' };

/** 座位名截断（沿用 rank.js 先例：超长昵称防压按钮） */
...
export default class LobbyScene {
  ...
  showFatal(msg, onClose) {
    this.fatalError = msg || tr('lobby.connect-fail');
    ...
    for (const r of this.tapRects) {
      if (px >= r.x && px <= r.x + r.w && py >= r.y && py <= r.y + r.h) {
        if (r.id === 'back') { this.room.leave(); this.onLeave(); }
          ...
          else this.showToast(tr('lobby.need-two'));
        }
  ...
  render() {
    ...
    ctx.fillText(tr('lobby.title'), cx, 90 * u);
    ...
    ctx.fillText(tr('lobby.room-hint'), cx, rcY + 24 * u);
    ctx.fillStyle = t.textColor;
    ...
    // 邀请按钮
    const iw = 200 * u;
    drawButton(ctx, t, { x: cx - iw / 2, y: rcY + 80 * u, w: iw, h: 50 * u, label: tr('lobby.invite'), primary: true });
    ...
    for (let i = 1; i <= 4; i += 1) {
      ...
      const name = type === 'human' ? clipName((this.room.roster.find((s) => s.seat === i) || {}).name || tr('lobby.seat.human') + i, 8) : type === 'ai' ? clipName(tr('lobby.ai-fill'), 8) : tr('lobby.seat.empty');
      ...
      if (this.room.isHost && (type === 'ai' || type === 'empty')) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(type === 'ai' ? tr('lobby.ai-remove') : tr('lobby.ai-fill'), tx + tw / 2, ty + th / 2 + 5 * u);
    ...
    drawButton(ctx, t, { x: 24 * u, y: by, w: backW, h: 56 * u, label: tr('lobby.back') });
    this.tapRects.push({ id: 'back', x: 24 * u, y: by, w: backW, h: 56 * u });
    if (this.room.isHost) {
      drawButton(ctx, t, { x: W - 24 * u - startW, y: by, w: startW, h: 56 * u, label: tr('lobby.start'), primary: true });
      ...
      ctx.textAlign = 'right';
      ctx.fillText(tr('lobby.wait-host'), W - 24 * u, by + 34 * u);
    }
    ...
    if (this.fatalError) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(tr('lobby.fatal-title'), cx, py + 52 * u);
      ctx.font = `bold ${Math.round(18 * u)}px sans-serif`;
      ctx.fillStyle = '#FF3B30';
      ctx.fillText(this.fatalError, cx, py + 92 * u);
      const bw2 = 180 * u;
      drawButton(ctx, t, { x: cx - bw2 / 2, y: py + 126 * u, w: bw2, h: 54 * u, label: tr('lobby.fatal-ok'), primary: true });
